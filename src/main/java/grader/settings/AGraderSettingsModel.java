...
//		 ModuleUserChange.newCase(currentModule, this, this);
	...
	String noDownloadPathMessage() {
		return "No stored download path. When the settings window comes up, please enter correct download path for a problem in module:"
				+ currentModule + " or change the module.";
	}
...
//            JOptionPane.showMessageDialog(null, "No stored download path. When the settings window comes up, please enter correct download path for a problem in module:" + currentModule + " or change the module.");
	...
	String noValidDownloadPathMessage() {
		return "No folder found for download path:" + problemDownloadPath
				+ ". In the settings window, please enter correct download path for a problem in module:"
				+ currentModule + " or change the module.";
	}
...
//            JOptionPane.showMessageDialog(null, "No folder found for download path:" + problemDownloadPath + ". In the settings window, please enter correct download path for a problem in module:" + currentModule + " or change the module.");
	...
	public void refreshAll() {
		boolean dynamicConfigurationHasProblem = false;
		loadDynamicConfigurationSettings();
...
//				Tracer.error("No folder found for:" + downloadPath);				
				...
				File gradesFile = new File(
						problemDownloadPath + "/" + ASakaiGradeSpreadsheetExtractor.GRADES_SPREADSHEET_NAME); // is this
																												// a
																												// sakai
																												// assignment
																												// folder
				if (!gradesFile.exists()) {
					gradesFile = new File(
							problemDownloadPath + "/" + ASakaiGradeSpreadsheetExtractor.ANON_GRADES_SPREADSHEET_NAME); // is
																														// this
																														// a
																														// sakai
																														// assignment
...
//                    }
					// addition
					if (currentProblem == null) {
						currentProblem = ConfigurationManagerSelector.getConfigurationManager()
								.getDynamicConfiguration().getString(StaticConfigurationUtils.PROBLEM_NAME);
//                	aFolderName = folder.getName();
						if (currentProblem == null) {
							currentProblem = folder.getName();
						} else {
							dynamicConfigurationHasProblem = true;

						}
						basicSetCurrentProblem(currentProblem);
...
//				currentProblem;
				for (File child : children) {
					if (child.getName().startsWith(currentModulePrefix) && !child.getName().equals("AssignmentsData") && 
							!dynamicConfigurationHasProblem) { // added 7/8/2023
...
//				String savedProblem = currentProblem;
			moduleProblemSelector.getProblem().setChoices(problems, currentProblem); // it is the same object but we
																						// need to fire property change
...
//        }
		...
		navigationSetter.getAutomaticNavigationSetter()
				.setAnimateGrades(graderSettingsManager.getAnimateGrades(aModule));

		navigationSetter.getAutomaticNavigationSetter()
				.setAnimationPauseTime(graderSettingsManager.getAnimationPauseTime(aModule));
...
//        GraderSettings.get().set("diff", diff);
		...
		graderSettingsManager.setAnimateGrades(aPrimaryModule,
				navigationSetter.getAutomaticNavigationSetter().getAnimateGrades());

		graderSettingsManager.setAnimationPauseTime(aPrimaryModule,
				navigationSetter.getAutomaticNavigationSetter().getAnimationPauseTime());

		String navigationFilter = navigationSetter.getNavigationFilterSetter().getNavigationFilterType().getValue()
				.toString();
		graderSettingsManager.setNavigationFilter(aPrimaryModule, navigationFilter);
		graderSettingsManager.setNavigationFilterOption(aPrimaryModule, navigationFilter,
				navigationSetter.getNavigationFilterSetter().getParameter());
